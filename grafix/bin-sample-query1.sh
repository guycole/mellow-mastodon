...
#
psql $PGDATABASE -c "select freq_hz, signal_dbm from mastodon_v1.bin_sample where row_head_id < 5 order by freq_hz;" > bin-sample1.txt
tr -d '|' < bin-sample1.txt > bin-sample1.dat
gnuplot -p bin-sample1.gp
#
psql $PGDATABASE -c "select freq_hz, rolling_mean, signal_dbm from mastodon_v1.bin_sample where row_head_id < 5 order by freq_hz;" > bin-sample2.txt
